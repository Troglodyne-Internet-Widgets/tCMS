...
sub serve ( $fullpath, $path, $start, $streaming, $ranges, $last_fetch = 0, $deflate = 0 ) {
    ...
    if ( open( my $fh, '<', $path ) ) {
        ...
        push( @headers, "Content-Length" => length(encode('UTF-8', $dfh)) );

        INFO("GET 200 ".length(encode('UTF-8', $dfh))." $fullpath");
...
sub _range ( $fullpath, $fh, $ranges, $sz, %headers ) {
    ...
    if ($is_multipart) {
        foreach my $range (@$ranges) {
            $headers{'Content-Length'} += length(encode('UTF-8', "$fc--$Trog::Vars::CHUNK_SEP\n$primary_ct\nContent-Range: bytes $range->[0]-$range->[1]/$sz\n\n"));
            $fc = "\n";
        }
        $headers{'Content-Length'} += length(encode('UTF-8', "\n--$Trog::Vars::CHUNK_SEP\--\n"));
        $fc = '';
    }
